...
{
    class ColumnarTranspositionAlgorithm : ICryptingAlgorithm
    {
        ...
        {
            input = (input.Length % key.Length == 0) ? input : input.PadRight(input.Length - (input.Length % key.Length) + key.Length, '-');
            StringBuilder output = new StringBuilder();
            int totalChars = input.Length;
